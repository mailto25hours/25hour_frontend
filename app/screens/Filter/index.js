...
export default function Filter({navigation,route}) {
  ...
  const [category, setCategory] = useState([
    {id: '1', name: 'Mobile Accessories', checked: true},
  ...
  const filterOptions = route.params.filterOptions;
  ...
  const saveFilter = () =>{
    //  console.log(category, interio,priceBegin,priceEnd,"selected")
     if (!loading) {
      setLoading(true);
     
      setTimeout(() => {
        setLoading(false);
        filterOptions(category,priceBegin,priceEnd)
        navigation.goBack();
      }, 500);
    }
  }
